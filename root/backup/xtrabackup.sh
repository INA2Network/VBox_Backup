...
if [ "$1" = "full" ]; then
        ...
        if [[ -d "$BACKUP" ]]; then
                ...
                if [[ "$BACKUP" == *full* ]]; then
                        ...
                        $RSYNC_COMMAND --quiet -ah --delete $BACKUP/ $DESTINATION 1>> $LOG_FILE 2>> $LOG_FILE || fail
                        echo -e "...done.\n"

                        echo "Preparing the destination for use with MySQL..."
                        $INNOBACKUPEX_COMMAND --apply-log --ibbackup=xtrabackup_51 $DESTINATION 1>> $LOG_FILE 2>> $LOG_FILE || fail
                        echo -e "...done.\n"
                else
      ...
      XTRABACKUP_COMMAND="$(which nice) -n 15 $IONICE_COMMAND $XTRABACKUP"
                        ...
                        echo "Copying data files to destination..."
                        $RSYNC_COMMAND --quiet -ah --delete $FULL_BACKUP/ $DESTINATION 1>> $LOG_FILE 2>> $LOG_FILE || fail
                        echo -e "...done.\n"
      ...
      $INNOBACKUPEX_COMMAND --apply-log --redo-only $DESTINATION 1>> $LOG_FILE 2>> $LOG_FILE || fail
                        echo -e "...done.\n"
        ...
        $INNOBACKUPEX_COMMAND --apply-log --redo-only $DESTINATION --incremental-dir=$INCREMENTAL 1>> $LOG_FILE 2>> $LOG_FILE || fail
                                echo -e "...done.\n"
                        done

                        echo "Finalising the destination..."
                        #$XTRABACKUP_COMMAND --prepare --target-dir=$DESTINATION  &>> $LOG_FILE || fail
      $INNOBACKUPEX_COMMAND --apply-log  $DESTINATION 1>> $LOG_FILE 2>> $LOG_FILE || fail
                        echo -e "...done.\n"
                fi
